...
def register_zbccl(local_rank, rank, world_size):
    ...
    zbccl_init(world_size, rank, local_rank, device_mem_size=local_mem_size, 
        ccl_meta_space_size=local_meta_size, 
        ip_port=G_IP_PORT)
    meta_addr = 0

    npu_tensor = torch.zeros(10, device="npu")
    print(npu_tensor)
    print(f"rank[{rank}] register zbccl, {meta_addr=}")
...
def init_dist(local_rank: int, num_local_ranks: int):
    ...
    register_zbccl(local_rank, global_rank, world_size)
