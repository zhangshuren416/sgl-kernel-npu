 ...
 */
#include "zbccl_kernel_reduce_scatter_zero_buff_kernel_new.h"


using namespace zbccl;
using namespace zbccl::reduce_scatter_zero_buff;

extern "C" __global__ __aicore__ void ShmemZeroBuffReduceScatterNew(
    ...
    int teamId, uint32_t reduceOp)
{
    ...
    switch (zcclDataType) {
        case ZCCLDataType::ZCCL_DATA_TYPE_INT32: {
            ZeroBuffReduceScatterKernelNew<int32_t> op;
            op.Init(input, output, gva, &pipe, rank, rankSize, totalLength, magic, fftsAddr, reduceOp);
            op.Process();
            break;
        }
        case ZCCLDataType::ZCCL_DATA_TYPE_FP32: {
            ZeroBuffReduceScatterKernelNew<float> op;
            op.Init(input, output, gva, &pipe, rank, rankSize, totalLength, magic, fftsAddr, reduceOp);
            op.Process();
            break;
        }
        case ZCCLDataType::ZCCL_DATA_TYPE_FP16: {
            ZeroBuffReduceScatterKernelNew<float16_t> op;
            op.Init(input, output, gva, &pipe, rank, rankSize, totalLength, magic, fftsAddr, reduceOp);
            op.Process();
            break;
        }
        case ZCCLDataType::ZCCL_DATA_TYPE_BFP16: {
            ZeroBuffReduceScatterKernelNew<bfloat16_t> op;
            op.Init(input, output, gva, &pipe, rank, rankSize, totalLength, magic, fftsAddr, reduceOp);
            op.Process();
            break;
        }
