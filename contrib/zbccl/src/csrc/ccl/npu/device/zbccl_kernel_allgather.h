...
{
public:
aicore inline AllGatherKernel() {}

template<typename T>
__aicore__ inline void Process(GM_ADDR input, GM_ADDR output, GM_ADDR comm, uint64_t elements, uint64_t ffts, GM_ADDR metaInfo)
{
    auto *commMeta = reinterpret_cast<__gm__ zbccl:ccl::ZCommMetaInfo*>(comm);
    AllGatherOrigin<T>(input, output, commMeta, elements);
}

template<typename T>
__aicore__ inline void AllGatherOrigin(GM_ADDR inputGM, GM_ADDR outputGM, zbccl:ccl::ZCommMetaInfo* commMeta, uint64_t elements)
{        
#ifdef DAV_C220_VEC
    shmem_barrier_all();
    ...
    GM_ADDR metaGM = (GM_ADDR)commMeta->myMetaDataGva;
