...
public:
    ...
    ZBCCL_KERNEL void Process(GM_ADDR input, GM_ADDR output, GM_ADDR metaGM, uint64_t elements)
    {
        ...
        __gm__ uint32_t *out_ptr = reinterpret_cast<__gm__ uint32_t *>(output);
        *out_ptr = groupSize;

        zbccl_barrier_all(myGroupRank, groupSize, localDeviceMemSize, &(comm->counter), output);
        *(out_ptr + 1) = myGroupRank;
        *(out_ptr + 2) = localDeviceMemSize;
        ...
        // AscendC::DumpTensor(meta_addr_tensor, 58, groupSize);
