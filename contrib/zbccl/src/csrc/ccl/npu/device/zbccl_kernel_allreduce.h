...
ZBCCL_KERNEL __gm__ void *GetRemotePtr(__gm__ void *ptr, int32_t curRank, int32_t dstRank,
                                              __gm__ uint16_t *peerGroupRank2WorldRank, uint64_t localMemSize)
{
    int32_t curGlobalRank = peerGroupRank2WorldRank[curRank];
    int32_t dstGlobalRank = peerGroupRank2WorldRank[dstRank];
    return (__gm__ void*)((__gm__ uint8_t*)ptr + localMemSize * (dstGlobalRank - curGlobalRank));
}

template <typename T>
ZBCCL_KERNEL void SetGmVal(__gm__ void *addr, T val)
{
    __gm__ T* typeAddr = (__gm__ T*)addr;
    *typeAddr = val;
    dcciCacheline((__gm__ uint8_t *)typeAddr);
}

template <typename T>
ZBCCL_KERNEL T GetGmVal(__gm__ void *Addr)
{
                                      ...
                                      uint64_t localDeviceMemSize)
{
    ...
    if (aivIndex < groupSize) {
        ...
        SetFlag(zbccl_ptr(metaAddr, rank, aivIndex, localDeviceMemSize), 1, rank, groupSize);
    }
}
