...
template<class T>
int test_shmem_allreduce(int rank_id, int n_ranks, uint64_t local_mem_size, zbccl_datatype_t dataType, bool zero_buff)
{
    // 初始化ACL和SHMEM
    ...
    for (int i = 0; i < test_cases.size(); i++) {
        ...
        std::string inputFile = cwd + "/golden/allreduce_" + std::to_string(trans_size) + "_" +
        ...
        void *output_ptr;
        size_t outSingleSize = trans_size * sizeof(T);
        if (zero_buff) {
        ...
        for (int zz = 0; zz < PERF_TIMES; zz++) {
            if (zero_buff) {
                // tmp debug
                zbccl_all_reduce(input_ptr, output_ptr, trans_size, dataType, reduceOp, teamId, stream);
            } else {
                zbccl_all_reduce(input_ptr, output_ptr, trans_size, dataType, reduceOp, teamId, stream);
            }
        }
        ...
        std::string goldenFile = cwd + "/golden/allreduce_" +
...
int main(int argc, char *argv[])
{
    ...
    if (std::string(data_type) == "int") {
        ...
        status = test_shmem_allreduce<int>(rank_id, n_ranks, local_mem_size, dataType, zero_buff);
    } else if (std::string(data_type) == "float") {
        status = test_shmem_allreduce<float>(rank_id, n_ranks, local_mem_size, dataType, zero_buff);
    } else if (std::string(data_type) == "float16") {
        dataType = zbccl_datatype_t::ZCCL_DATA_TYPE_FP16;
        status = test_shmem_allreduce<float16>(rank_id, n_ranks, local_mem_size, dataType, zero_buff);
    } else if (std::string(data_type) == "bfloat16") {
        dataType = zbccl_datatype_t::ZCCL_DATA_TYPE_BFP16;
        status = test_shmem_allreduce<bfloat16>(rank_id, n_ranks, local_mem_size, dataType, zero_buff);
    }
